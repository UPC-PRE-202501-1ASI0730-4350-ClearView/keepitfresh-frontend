...
import { getSensorStatusStats } from '/src/shared/services/inventory.service.js'
...
onMounted(async () => {
  chartData.value = await getSensorStatusStats()
})

const exportSensorStatusPDF = async () => {
  const chartElement = document.getElementById('sensor-status-chart')
  ...
  pdf.save('sensor-status.pdf')
}

const exportSensorStatusCSV = async () => {
